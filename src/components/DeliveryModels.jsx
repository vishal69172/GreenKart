import React from 'react';
import { FiTruck, FiZap } from 'react-icons/fi';

const DeliveryModels = () => {
  return (
    <section className="py-20 bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-4">
            Our Delivery Models
          </h2>
          <p className="text-lg text-gray-600 max-w-3xl mx-auto leading-relaxed">
            We offer flexible delivery options tailored to your needs—whether it's next-day restocking, urgent same-day supplies, or specialty products.
          </p>
        </div>

        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-10 text-center shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-blue-100">
            <div className="text-6xl text-blue-600 mb-6 flex justify-center">
              <FiTruck />
            </div>
            <h3 className="text-3xl font-bold text-gray-900 mb-4">
              Wholesale Delivery
            </h3>
            <p className="text-gray-700 text-lg leading-relaxed">
              Perfect for regular restocking with next-day delivery options. Ideal for bulk orders and planned inventory management.
            </p>
          </div>

          <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl p-10 text-center shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-orange-100">
            <div className="text-6xl text-orange-600 mb-6 flex justify-center">
              <FiZap />
            </div>
            <h3 className="text-3xl font-bold text-gray-900 mb-4">
              Express Delivery
            </h3>
            <p className="text-gray-700 text-lg leading-relaxed">
              Urgent same-day supplies for when you need ingredients fast. Get your specialty products delivered quickly.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
};

export default DeliveryModels;



